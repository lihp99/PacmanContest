...
class OffensiveAstarAgent(CaptureAgent):
    ...
    def choose_action(self, game_state):
            ...
            # set goal to closest power capsule
            
            goal = min(food_list, key=lambda food: self.get_maze_distance(my_pos, food))
            path = self.astar_search(game_state, my_pos, goal)

            food_left = len(self.get_food(game_state).as_list())
            if food_left <= 2 or self.get_food_you_are_defending(game_state).count() <= 9:
                print("ASTAR AGENT: less than 2 pellets left - DEFEND!")
                best_dist = 9999
                best_action = None
    ...
    # functions that we need for the defensive part:
...
class OurTestAgent(CaptureAgent):
    ...
    def register_initial_state(self, game_state):
        ...
        self.height, self.width = walls.height, walls.width

        self.red = game_state.is_on_red_team(self.index)
        self.register_team(self.get_team(game_state))
        if self.red:
            print("RED")
            # set self.home to half of self.width but round down
            self.home = self.width // 2
        else:
            print("BLUE")
            self.home = self.width // 2 + 1
    ...
    def choose_action(self, game_state):
        ...
        # print amount of pellets left on our side
        # print(f"pellets left:{len(self.get_food(game_state).as_list())}")

        # if we are scared, we go into sneaky mode
        if score < 0:
            self.mode = 'sneaky_pellet'
        ...
        if self.mode == 'get_home':
            ...
            if food_left <= 2:
                print("TEST AGENT: less than 2 pellets left")
                best_dist = 9999
                best_action = None
                for action in actions:
                    successor = self.get_successor(game_state, action)
    ...
    def park_bus_features(self, game_state, action):
        ...
        # add feature that makes the agent more likely to stay in lower / upper half of board depending on it's index
        # if self.index % 2 == 0:
        #     print("stay in lower half")
        #     print(my_pos)
        #     features['height'] = -my_pos[0]
        # else:
        #     print("stay in upper half")
        #     print(my_pos)
        #     features['height'] = my_pos[0]
...
class OurTestAgentMARK2(CaptureAgent):
    ...
    def register_initial_state(self, game_state):
        ...
        walls = game_state.get_walls()
        self.height, self.width = walls.height, walls.width
        self.oponents = self.get_opponents(game_state)
        print(f"oponents:{self.oponents}")
    ...
    def choose_action(self, game_state):
        ...
        if len(self.get_food(game_state).as_list()) <= 2 or self.get_food_you_are_defending(game_state).count() <= 15:
            # print("MARK 2 DEFEND!!!")
            self.mode = 'park_the_bus'
        # check if the oter team is scared

        



        # if game_state.get_agent_state(self.index).scared_timer > 20:
        #     print("MARK 2: scared")
        #     print(game_state.get_agent_state(self.index).scared_timer )
        #     self.mode = 'sneaky_pellet'
        ...
        if self.mode == 'get_home':
            ...
            # print("MARK 2: going to get a pellet")
            if pos in self.food_list:
                self.mode = 'get_home'
                self.food_list = self.get_food(game_state).as_list()
                return random.choice(game_state.get_legal_actions(self.index))
            ...
            # return self.minimax_gen.get_next_action(game_state)
            # print("PARK THE BUS")
            # while current position x value is not equal or bigger to home value, set goals to home
            if pos[0] < self.home:
                goals = [(self.home, i) for i in range(self.height)]
            else:
                # set goals to nearest intruder position
                print("MARK 2: set goals to nearest intruder position")
                action = game_state.get_legal_actions(self.index)[0]
                successor = self.get_successor(game_state, action)
                enemies = [successor.get_agent_state(i) for i in self.get_opponents(successor)]
                invaders = [a for a in enemies if a.is_pacman and a.get_position() is not None]
                goals = [a.get_position() for a in invaders]
    ...
    def park_bus_weights(self, game_state, action):
        return {'num_invaders': -1000, 'on_defense': 100, 'invader_distance': -10, 'stop': -100, 'reverse': -5,}
